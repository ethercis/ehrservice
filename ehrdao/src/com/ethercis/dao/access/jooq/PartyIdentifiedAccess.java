...
//    }
    ...
    @Override
    public Integer addIdentifier(String idCode, String issuer, String assigner, String typeName){

        IdentifierRecord identifierRecord = context.newRecord(IDENTIFIER);
        identifierRecord.setIdValue(idCode);
        identifierRecord.setIssuer(issuer);
        identifierRecord.setAssigner(assigner);
        identifierRecord.setTypeName(typeName);

        if (identifiers == null){
            identifiers = new HashMap<>();
        }
        identifiers.put(makeMapKey(idCode, issuer), identifierRecord);
    ...
    private static UUID findReferencedParty(DSLContext context, PartyRef partyRef){
