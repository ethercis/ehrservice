...
public class ContextAccess extends DataAccess implements I_ContextAccess {
    ...
    public void setRecordFields(UUID id, EventContext eventContext) throws Exception {
        ...
        eventContextRecord.setEndTime((eventContext.getEndTime() != null ? new Timestamp(eventContext.getEndTime().getDateTime().getMillis()) : null));
...
//                String type = performer.getExternalRef().getType();
    ...
    public UUID commit(Timestamp transactionTime) throws Exception {
        ...
        String sql = "INSERT INTO ehr.event_context (composition_id, start_time, end_time, facility, location, other_context, setting, sys_transaction) " +
                "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?)" +
                " RETURNING id";
        ...
        insertStatement.setObject(3, eventContextRecord.getEndTime());
        insertStatement.setObject(4, eventContextRecord.getFacility());
        insertStatement.setObject(5, eventContextRecord.getLocation());
        insertStatement.setObject(6, eventContextRecord.getOtherContext());
        insertStatement.setObject(7, eventContextRecord.getSetting());
        insertStatement.setObject(8, eventContextRecord.getSysTransaction());
...
//        eventContextRecord.changed(EVENT_CONTEXT.OTHER_CONTEXT, false);
        ...
        updateStatement.setObject(3, eventContextRecord.getEndTime());
        updateStatement.setObject(4, eventContextRecord.getFacility());
        updateStatement.setObject(5, eventContextRecord.getLocation());
        updateStatement.setObject(6, eventContextRecord.getOtherContext());
        updateStatement.setObject(7, eventContextRecord.getSetting());
        updateStatement.setObject(8, eventContextRecord.getSysTransaction());
...
//    }
    ...
    private static DvDateTime decodeDvDateTime(Timestamp timestamp){
        if (timestamp == null) return null;
        DateTime codedDateTime = new DateTime(timestamp);
    ...
    public EventContext mapRmEventContext(){
        ...
        context.fetch(PARTICIPATION, PARTICIPATION.EVENT_CONTEXT.eq(eventContextRecord.getId())).forEach(record -> {
            ...
            DvInterval<DvDateTime> startTime = new DvInterval<>(decodeDvDateTime(record.getStartTime()), null);
        ...
        return new EventContext(healthCareFacility,
                decodeDvDateTime(eventContextRecord.getStartTime()),
                decodeDvDateTime(eventContextRecord.getEndTime()),
                participationList.isEmpty() ? null: participationList,
                eventContextRecord.getLocation() == null ? "default" : eventContextRecord.getLocation(),
                concept,
                null, //other context...
                terminologyService);
    ...
    public static EventContext retrieveHistoricalEventContext(I_DomainAccess domainAccess, UUID compositionId, Timestamp transactionTime){
        ...
        return new EventContext(healthCareFacility,
                decodeDvDateTime(eventContextHistoryRecord.getStartTime()),
                decodeDvDateTime(eventContextHistoryRecord.getEndTime()),
                participationList.isEmpty() ? null: participationList,
                eventContextHistoryRecord.getLocation() == null ? "default" : eventContextHistoryRecord.getLocation(),
                concept,
                null, //other context...
                terminologyService);
