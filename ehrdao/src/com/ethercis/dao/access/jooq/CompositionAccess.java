...
//            });
        ...
        if (force){ //updateComposition event context accordingly
            //retrieve context and force updateComposition it
    ...
    public Integer delete() {
        ...
        int count = 0;
        //delete composition
        log.debug("deleting composition id:"+compositionRecord.getId());
        count += compositionRecord.delete();

        log.debug("Event context and participation deleted:"+count);
    ...
    public static I_CompositionAccess retrieveCompositionVersion(I_DomainAccess domainAccess, UUID id, int version) throws Exception {
        String versionQuery =
                "select ehr_id, language, territory, composer, sys_transaction from \n" +
        ...
        if (compositionHistoryAccess != null)
            compositionHistoryAccess.setContent(I_EntryAccess.retrieveInstanceInCompositionVersion(domainAccess, compositionHistoryAccess));
...
//    }
    ...
    public static I_CompositionAccess retrieveInstance(I_DomainAccess domainAccess, UUID id) throws Exception {
        ...
        compositionAccess.setCompositionRecord(domainAccess.getContext().selectFrom(COMPOSITION).where(COMPOSITION.ID.eq(id)).fetchOne());
