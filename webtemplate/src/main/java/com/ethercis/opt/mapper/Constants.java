package com.ethercis.opt.mapper;/*
 * Copyright (c) Ripple Foundation CIC Ltd, UK, 2017
 * Author: Christian Chevalley
 * This file is part of Project Ethercis
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

/**
 * Created by christian on 2/12/2018.
 */
public class Constants {
    public static final String VALIDATION = "validation";
    public static final String NODE_ID = "node_id";
    public static final String AQL_PATH = "aql_path";
    public static final String MIN_OP = "min_op";
    public static final String LIMITS = "limits";
    public static final String VALUE = "value";
    public static final String MAX_OP = "max_op";
    public static final String MAX = "max";
    public static final String TYPE = "type";
    public static final String CONSTRAINT = "constraint";
    public static final String CHILDREN = "children";
    public static final String NAME = "name";
    public static final String DESCRIPTION = "description";
    public static final String MIN = "min";
    public static final String SUFFIX = "suffix";
    public static final String UNITS = "units";
    public static final String LABEL = "label";
    public static final String DEFINING_CODE = "defining_code";
    public static final String VALUE_MAP = "value_map";
    public static final String TERMINOLOGY = "terminology";
    public static final String RM_TYPE = "rm_type";
    public static final String ASSUMED_VALUE = "assumed_value";
    public static final String SYMBOL = "symbol";
    public static final String CODE = "code";
    public static final String NORMAL_STATUS = "normal_status";
    public static final String ARCHETYPE_NODE_ID = "archetype_node_id";
    public static final String ARCHETYPE_DETAILS = "archetype_details";
    public static final String DATA = "data";
    public static final String ATTRIBUTE = "attribute";
    public static final String CONTEXT = "context";
    public static final String PROPERTY = "property";
    public static final String MAGNITUDE = "magnitude";
    public static final String FALSE_VALID = "false_valid";
    public static final String TRUE_VALID = "true_valid";
    public static final String NUMERATOR = "numerator";
    public static final String DENOMINATOR = "denominator";
    public static final String VALUES = "values";
    public static final String UPPER = "upper";
    public static final String LOWER = "lower";
    public static final String PATTERN = "pattern";
    public static final String OCCURRENCE = "occurrence";
    public static final String SQL_PATH = "ethercis_sql";
    public static final String MANDATORY_ATTRIBUTES = "mandatory_attributes";

    public static final String COMPOSITION = "COMPOSITION";
    public static final String STRING = "STRING";
    public static final String BOOLEAN = "BOOLEAN" ;
    public static final String INTEGER = "INTEGER" ;
    public static final String REAL = "REAL" ;
    public static final Object MULTIPLE = "MULTIPLE";
    public static final String PARTY_PROXY = "PARTY_PROXY";
    public static final String PARTY_REF = "PARTY_REF";
    public static final String CURRENT_STATE = "current_state";
    public static final String CAREFLOW_STEP = "careflow_step";
    public static final String CODE_STRING = "code_string";
    public static final String TERM_BINDING = "terminology_binding";
    public static final String TEMPLATE_ID = "template_id";
    public static final String DEFAULT_LANGUAGE = "default_language";
    public static final String LANGUAGES = "languages";
    public static final String UID = "uid";
    public static final String CONCEPT = "concept";
    public static final String TREE = "tree";
    public static final String ATTRIBUTE_NAME = "attribute_name";
    public static String PROPORTION_KIND = "type";
}
