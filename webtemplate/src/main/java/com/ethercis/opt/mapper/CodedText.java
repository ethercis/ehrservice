...
import com.ethercis.opt.SimpleTerminology;
            ...
            for (COBJECT cobject : defining_codes) {
                if (cobject instanceof CCODEPHRASE) {
                    ...
                    for (String code : ((CCODEPHRASE) cobject).getCodeListArray()) {
                        ...
                        else if ("openehr".equals(((CCODEPHRASE) cobject).getTerminologyId().getValue())){
                            codeMap.put(Constants.VALUE, new SimpleTerminology(code).label());
                        }
