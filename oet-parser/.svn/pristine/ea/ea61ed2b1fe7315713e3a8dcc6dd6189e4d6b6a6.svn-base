...
import openEHR.v1.template.impl.CLUSTERImpl;
...
public class FlattenerNew {
	...
	private Archetype flattenItem(Archetype parentArchetype, ITEM item) 
        ...
        //flatten nested Item
        //CHC: added recursion for nested items

        if (item instanceof CLUSTERImpl) {

            log.debug("iterating sub-item: "+item.getPath()+" "+item.getName()+" "+item.getArchetypeId());

            ITEM[] items = ((CLUSTERImpl) item).getItemsArray();

            for (ITEM subItem: items){
                flattenItem(archetype, subItem);
            }
        }
	...
	private static Logger log = Logger.getLogger(FlattenerNew.class);
